...
const { data: players } = await useFetch<Player[]>('/api/roster')
const { data: teams } = await useFetch<Team[]>('/api/teams')

const player = computed(() => players.value?.find(p => p.id.toString() === playerId))
...
const state = reactive<Partial<Schema>>({
  ...
  jerseyNumber: player.value.jerseyNumber,
  birthDate: player.value.birthDate,
  status: player.value.status,
  avatar: player.value.avatar?.src || ''
})
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    toast.add({
      title: 'Success',
      description: `Player "${event.data.name}" has been updated successfully!`,
      color: 'success'
    })
...
function deletePlayer() {
  ...
  toast.add({
    ...
    description: 'Delete player functionality would be implemented here',
    color: 'warning'
  })
}
...
<template>
  <UDashboardPanel>
    <template #header>
      <UDashboardNavbar :title="`Edit ${player?.name}`">
        <template #leading>
          <UButton 
      ...
      <UCard class="max-w-2xl mx-auto">
        <template #header>
          <div class="flex items-center space-x-2">
            ...
            <h2 class="text-xl font-semibold">Edit Player Information</h2>
          </div>
        </template>
